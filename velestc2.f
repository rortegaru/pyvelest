...
c        call CPUTIMER(cpusec)
         ...
         write(bo,'(x,''>>>   End of program VELEST at '',a20,
     &           '' <<<'','' [ CPU-sec : '',f7.1,'']'')') '------',0
       call adlineb(bo)
         endif
...
c
      if(.not.single_turbo)
     &              write(bo,*)'~~~ set units for first iteration ...'
       call adlineb(bo)
...
c
      if(.not.single_turbo)
     &           write(bo,*)'~~~ determine number of unknowns ...'
       call adlineb(bo)
      call DETNOFUNKNOWNS(bo)
c
c     reset G-matrix and RHT-vector :
c
      if(.not.single_turbo) write(bo,*)'~~~ reset G and RHT ...'
       call adlineb(bo)
      do j=1,kvar
         g(j)=0.0
      enddo
...
c
         do l=1,k
            call TRAVELTIME(i,l,iresflag,bo)
     ...
     +            '' nvar='',i2)') knobs(i),nobswithw0,nvar
            ...
            if(.not.single_turbo)then
               write(bo,*)'~~~ compute singular values of G ...'
       call adlineb(bo)
            endif
            call SINGULARVALUES(i,bo)
         endif
      enddo
c
...
c
      if(.not.single_turbo) write(bo,*)'~~~ compute RMS and DATVAR ...'
       call adlineb(bo)
      call RMSDATVAR(bo)
c
      if(ittmax.eq.0)then
...
c
      if(.not.single_turbo) write(bo,*)'~~~ save initial datvar ...'
       call adlineb(bo)
      call CHECKSOLUTION(istopflag,better,bo)
c
c
...
c
      if(.not.single_turbo)then
         write(bo,*)'~~~ set units for this iteration ...'
       call adlineb(bo)
      endif
...
c
      if(.not.single_turbo)then
      write(bo,*)'~~~ determine number of unknowns for this iter. ...'
       call adlineb(bo)
      endif
      call DETNOFUNKNOWNS(bo)
c
...
c
      if(isingle.eq.0)then
         if(.not.single_turbo)then
            write(bo,*)'Damping factors to be used in this iteration:'
       call adlineb(bo)
...
c
      if(.not.single_turbo) write(bo,*)'~~~ damp G ...'
       call adlineb(bo)
      call DAMPG
c
...
c
      if(.not.single_turbo) write(bo,*)'~~~ solve G * b = RHT ...'
       call adlineb(bo)
...
c
      if(.not.single_turbo) write(bo,*)'~~~ fix units ...'
       call adlineb(bo)
...
c
      if(.not.single_turbo)then
         write(bo,*)'~~~ apply eventually step-length damping ...'
       call adlineb(bo)
      endif
...
c
      if(.not.single_turbo)then
      write(bo,*)'~~~ adjust model (hypocenters, stacorr & veloc.)...'
       call adlineb(bo)
      endif
      call ADJUSTMODEL(damp,bo)
c
c     calculate total step-length of this iteration and print it out:
c
      if(.not.single_turbo)then
         write(bo,*)'~~~ compute effective step-length ...'
       call adlineb(bo)
      endif
      call STEPLENGTHCALC(bo)
c
c
...
c
      if(ibackups.lt.4)then
         if(.not.single_turbo)then
            write(bo,*)'~~~ set units for next iteration ...'
       call adlineb(bo)
         endif
...
c
      if(.not.single_turbo)then
       if(ibackups.lt.4)then
       write(bo,*)'~~~ determine number of unknowns for next iter. ...'
       call adlineb(bo)
       else
       write(bo,*)'~~~ determine number of unknowns for this iter. ...'
       call adlineb(bo)
...
c
      if(irayout.eq.1) rewind(13) ! rewind file (if desired) with raypaths
      if(idrvout.eq.1) rewind(21) ! rewind file (if desired) with raypaths
      if(.not.single_turbo) write(bo,*)'~~~ raytracing, setup G ...'
       call adlineb(bo)
      do i=1,legs
         avres(i)=0.
         rms(i)=0.
...
c
      if(.not.single_turbo) write(bo,*)'~~~ store parts of G ...'
       call adlineb(bo)
      do k=1,4
         call STOREG(k,k)
      enddo
...
c
      if(.not.single_turbo)then
         write(bo,*)'~~~ set units back for current iteration ...'
       call adlineb(bo)
      endif
      call SETUNT(nitt,invertratio,nsinv,icount,
     &                 xythet,stathet,othet,vthet,zthet,scale)
...
c
      if(.not.single_turbo)then
         write(bo,*)'~~~ compute RMS and DATVAR ...'
       call adlineb(bo)
      endif
      call RMSDATVAR(bo)
c
...
c
      if(.not.single_turbo)then
         write(bo,*)'~~~ check solution (better? worse?) ...'
       call adlineb(bo)
      endif
      call CHECKSOLUTION(istopflag,better,bo)
c
      ...
      if(ibackups.lt.4)then
         if(better)then
            ...
            if(ibackups.eq.0)then
               if(.not.single_turbo)then
                  call NITTOUTPUT(damp,bo)
               endif
            endif
            if(ibackups.lt.4)then
               if(.not.single_turbo)then
                  write(bo,'('' msqrd res increased'',
...
c
      if(.not.single_turbo)then
         write(bo,*)'~~~ iteration done; output results ...'
       call adlineb(bo)
      endif
      if(.not.single_turbo) call NITTOUTPUT(damp,bo)
c
      if(.not.single_turbo) write(bo,*)'~~~ another iteration? ...'
       call adlineb(bo)
c
...
c
      if(ibackups.eq.4)then
         if(.not.single_turbo)then
            write(bo,*)'4 TIMES BACKUP MADE --> sTOP ITERATIONS.'
       call adlineb(bo)
         endif
         goto 9999
      endif
...
c
      if(steplen.gt.0)then  ! otherwise we are during backups...
         if(steplen.lt.delmin)then
            if(.not.single_turbo)then
               write(bo,*)'STEP-LENGTH < DELMIN  --> stop iterations.'
       call adlineb(bo)
            endif
            goto 9999
         endif
...
c
      if(nitt.eq.ittmax)then
         if(.not.single_turbo)then
            write(bo,*)'NITT = ITTMAX  --> stop iterations.'
       call adlineb(bo)
         endif
         goto 9999
      endif
...
c
      if(isingle.ne.0.and.istopflag.eq.1)then
         if(.not.single_turbo)then
        write(bo,*)'Changes in datvar < 1.e-6  --> stop iterations.'
       call adlineb(bo)
         endif
         goto 9999
      endif
...
c
     ...
     &      write(bo,*)'~~~ damp G ...'
       call adlineb(bo)
         ...
         if(.not.single_turbo)then
            write(bo,*)'~~~ Cholesky decomposition of G ...'
       call adlineb(bo)
         endif
...
ck            write(bo,59) (rms(i),i=1,legs)
       call adlineb(bo)
ck 59         format(' Final rms values: ',10f5.2,/,(19x,10f5.2))
ck            write(bo,'(//)')
       call adlineb(bo)
              do j=1,legs
              write(bo,59) j,rms(j)
       call adlineb(bo)
 59           format('#  ',i3,' rms value: ',10f5.2)
              enddo
         endif
      endif
c
     ...
     &                       regnamfile,regkoordfile) ! Subr. REGION needs LON E
      ...
      if(.not.single_turbo)then
         write(bo,*)'~~~ output final hypocenters ...'
       call adlineb(bo)
      endif
      call FINALHYPOCOUT(bo)
      if(.not.single_turbo)then
       if(isingle.ne.0)then
      write(bo,'('' Singular values:     '',4f10.4,5x,''ALE ='',f7.3)')
     &                (SV(j),j=1,4) , ale(1)
       call adlineb(bo)
...
c
      if(.not.single_turbo)then
         write(bo,*)'~~~ output final station residuals ...'
       call adlineb(bo)
         call FINALSTARESI(bo)
      endif
      if(iresolcalc.gt.0)then
         if(.not.single_turbo)then
       write(bo,*)'~~~ output resolution- & covariance matrices ...'
       call adlineb(bo)
         endif
         call RESOLCOVAR(davar1,bo)
      endif
...
c
      if(isingle.eq.0)then
         write(bo,*)'~~~ output statistics ...'
       call adlineb(bo)
         call STATISTICSOUT(bo)
      endif
c
...
c
      ...
      if(.not.single_turbo)then
         cpmintot=cpusec/60.
         write(bo,'(1x,''TOTAL CPU-sec      '',3x,'' ='',f7.1,5x,
...
c
         if(.not.single_turbo)then
        write(bo,*)'~~~ output solution in HYPO71-compatible format...'
       call adlineb(bo)
         endif
c
...
c
      goto 99999
c
  998 call OPENERROR('main program','data-input-file FOR008',bo)
c
99999 continue
      END
c
...
c        write(6,*) 'Antes del primer llamado '
         write(bo,'(a)') headerline(1)
       call adlineb(bo)
c        write(6,*) 'Despues del primer llamado'
         write(bo,*)
       call adlineb(bo)
...
c
     ...
     &          olat,olon,icoordsystem,zshift,itrial,ztrial,ised
      write(bo,'(a)')'***'
       call adlineb(bo)
      write(bo,'(a)')'*** neqs   nshot    rotate'
      write(bo,'(1x,i6,1x,i6,3x,f6.1)') neqs,nshot,rotate
       call adlineb(bo)
      write(bo,'(a)')'***'
      write(bo,'(a)')'*** isingle   iresolcalc'
       call adlineb(bo)
      write(bo,'(7x,i1,10x,i1)') isingle, iresolcalc
      write(bo,'(a)')'***'
       call adlineb(bo)
     ...
     &          dmax,itopo,zmininput,veladj,zadj,lowveloclay
       call adlineb(bo)
      write(bo,'(a)')'***'
      write(bo,'(a)')'*** nsp    swtfac   vpvs       nmod'
       call adlineb(bo)
      write(bo,'(5x,i1,5x,f5.2,3x,f6.3,7x,i2)') nsp, swtfac, vpvs, nmod
      write(bo,'(a)')'***'
       call adlineb(bo)
      write(bo,'(a)')'***   othet   xythet    zthet    vthet   stathet'
      write(bo,'(4x,5(f7.2,2x))') othet,xythet,zthet,vthet,stathet
       call adlineb(bo)
      write(bo,'(a)')'***'
     ...
     &                         iuseelev,iusestacorr
      write(bo,'(a)')'***'
       call adlineb(bo)
      write(bo,'(a)')'*** iturbo    icnvout   istaout   ismpout'
      write(bo,'(7x,i1,9x,i1,9x,i1,9x,i1)') iturbo,
     ...
     &                            irflout,irfrout,iresout
      ...
      write(bo,'(a)')'*** delmin   ittmax   invertratio'
       call adlineb(bo)
      write(bo,'(3x,f6.3,6x,i2,9x,i2)') delmin, ittmax, invertratio
      write(bo,'(a)')'***'
       call adlineb(bo)
      write(bo,'(a)')'*** Modelfile:'
      write(bo,'(a)') modelfilename(1:trimlen(modelfilename))
       call adlineb(bo)
      write(bo,'(a)')'***'
      write(bo,'(a)')'*** Stationfile:'
       call adlineb(bo)
      write(bo,'(a)') stationfilename(1:trimlen(stationfilename))
      write(bo,'(a)')'***'
       call adlineb(bo)
      write(bo,'(a)')'*** Seismofile:'
      write(bo,'(a)') seismofilename(1:trimlen(seismofilename))
       call adlineb(bo)
      write(bo,'(a)')'***'
      write(bo,'(a)')'*** File with region names:'
       call adlineb(bo)
      write(bo,'(a)') regnamfile(1:trimlen(regnamfile))
      write(bo,'(a)')'***'
       call adlineb(bo)
      write(bo,'(a)')'*** File with region coordinates:'
      write(bo,'(a)') regkoordfile(1:trimlen(regkoordfile))
       call adlineb(bo)
      write(bo,'(a)')'***'
      write(bo,'(a)')'*** File #1 with topo data:'
       call adlineb(bo)
      write(bo,'(a)') topo1file(1:trimlen(topo1file))
      write(bo,'(a)')'***'
       call adlineb(bo)
      write(bo,'(a)')'*** File #2 with topo data:'
      write(bo,'(a)') topo2file(1:trimlen(topo2file))
       call adlineb(bo)
      write(bo,'(a)')'***'
      write(bo,'(a)')'*** DATA INPUT files:'
       call adlineb(bo)
      write(bo,'(a)')'***'
      write(bo,'(a)')'*** File with Earthquake data:'
       call adlineb(bo)
      write(bo,'(a)') phasefile(1:trimlen(phasefile))
      write(bo,'(a)')'***'
       call adlineb(bo)
      write(bo,'(a)')'*** File with Shot data:'
      write(bo,'(a)') shotfile(1:trimlen(shotfile))
       call adlineb(bo)
      write(bo,'(a)')'***'
      write(bo,'(a)')'*** OUTPUT files:'
       call adlineb(bo)
      write(bo,'(a)')'***'
      write(bo,'(a)')'*** Main print output file:'
       call adlineb(bo)
      write(bo,'(a)') outfile(1:trimlen(outfile))
      write(bo,'(a)')'***'
       call adlineb(bo)
      write(bo,'(a)')'*** File with single event locations:'
      write(bo,'(a)') velfile(1:trimlen(velfile))
       call adlineb(bo)
      write(bo,'(a)')'***'
      write(bo,'(a)')'*** File with final hypocenters in *.cnv format:'
       call adlineb(bo)
      write(bo,'(a)') cnvfile(1:trimlen(cnvfile))
      write(bo,'(a)')'***'
       call adlineb(bo)
      write(bo,'(a)')'*** File with new station corrections:'
      write(bo,'(a)') stafile(1:trimlen(stafile))
       call adlineb(bo)
      write(bo,'(a)')'***'
      write(bo,'(a)')'*** File with summary cards (e.g. for plotting):'
       call adlineb(bo)
      write(bo,'(a)') smpfile(1:trimlen(smpfile))
      write(bo,'(a)')'***'
       call adlineb(bo)
      write(bo,'(a)')'*** File with raypoints:'
      write(bo,'(a)') rayfile(1:trimlen(rayfile))
       call adlineb(bo)
      write(bo,'(a)')'***'
      write(bo,'(a)')'*** File with derivatives:'
       call adlineb(bo)
      write(bo,'(a)') drvfile(1:trimlen(drvfile))
      write(bo,'(a)')'***'
       call adlineb(bo)
      write(bo,'(a)')'*** File with ALEs:'
      write(bo,'(a)') alefile(1:trimlen(alefile))
       call adlineb(bo)
      write(bo,'(a)')'***'
      write(bo,'(a)')'*** File with Dirichlet spreads:'
       call adlineb(bo)
      write(bo,'(a)') dsprfile(1:trimlen(dsprfile))
      write(bo,'(a)')'***'
       call adlineb(bo)
      write(bo,'(a)')'*** File with reflection points:'
      write(bo,'(a)') rflfile(1:trimlen(rflfile))
       call adlineb(bo)
      write(bo,'(a)')'***'
      write(bo,'(a)')'*** File with refraction points:'
       call adlineb(bo)
      write(bo,'(a)') rfrfile(1:trimlen(rfrfile))
      write(bo,'(a)')'***'
       call adlineb(bo)
      write(bo,'(a)')'*** File with residuals:'
      write(bo,'(a)') resfile(1:trimlen(resfile))
       call adlineb(bo)
      write(bo,'(a)')'***'
      write(bo,'(///)')
       call adlineb(bo)
...
c
      if(.not.single_turbo)then
         write(bo,*)'Origin of cartesian coordinates:'
       call adlineb(bo)
         if(olon.le.0.0)then
           write(bo,'(6x,f10.5,2hN ,f11.5,1hE)')olat,-olon
       call adlineb(bo)
         else
           write(bo,'(6x,f10.5,2hN ,f11.5,1hW)')olat,olon
       call adlineb(bo)
         endif
       ...
       call adlineb(bo)
         ...
         write(bo,*)
       call adlineb(bo)
         write(bo,*)' X,Y-AXES rotated clockwise from North'
         write(bo,*)'          Rotation angle (rotate):'
       call adlineb(bo)
         write(bo,'(12x,7hrotate=,1x,f6.1,1x,5hdegr.)')rotate
         write(bo,*)
       call adlineb(bo)
c
         if(itrial.gt.0)then
       ...
       call adlineb(bo)
            write(bo,*)' ============================================'
            write(bo,*)' trial epicenter ~ earliest station'
       call adlineb(bo)
            write(bo,*)' trial depth = ztrial = ',ztrial
            write(bo,*)' ============================================'
       call adlineb(bo)
            write(bo,*)
         endif
c
         if(icoordsystem.eq.2)then
            write(bo,*)'SWISS COORDINATES will be used instead of the',
     &                 ' short distance conversion!'
       call adlineb(bo)
            write(bo,*)'origin BERNE: x=600.km, y=200.km'
            write(bo,*)'x is positive towards east,',
...
ccc            write(bo,*)'---> no rotation can be performed !!!'
       ...
       call adlineb(bo)
            write(bo,*)'normal SHORT DISTANCE CONVERSION will be made'
            write(bo,*)'x is positive towards west,',
     &                 ' y is positive towards north'
       call adlineb(bo)
         endif
      endif
...
c
      if(dmax.eq.0.)then
         if(.not.single_turbo)then
            write(bo,*)'WARNING: dmax was zero ! ... set to 150 km !'
       call adlineb(bo)
         endif
         dmax=150.
      endif
c
      ...
      if(.not.single_turbo)then
         if(iuseelev.eq.0)then
            write(bo,*)
       call adlineb(bo)
            write(bo,*)'Station-elevations internally set to ZERO !'
       ...
       call adlineb(bo)
         ...
         if(iusestacorr.eq.0)then
            write(bo,*)
       call adlineb(bo)
           write(bo,*)'Station-corrections set to ZERO !'
           write(bo,*)'(if you do NOT invert for station-corrections,'
       call adlineb(bo)
           write(bo,*)' then these 0.0-values are printed in file12 !)'
           write(bo,*)
       call adlineb(bo)
         endif
      endif
...
c
      if(.not.single_turbo)then
       ...
       call adlineb(bo)
         ...
         write(bo,'(//)')
       call adlineb(bo)
         write(bo,*)'Model(s) read from file :'
         write(bo,*) modelfilename
       call adlineb(bo)
         write(bo,*)
      endif
c     close(10)
...
12       format(f5.2,5x,f7.2,2x,f7.3,3x,a1)
         ...
         if(reflch.eq.'m'.or.reflch.eq.'M')then
            ...
            if(.not.single_turbo)then
               if(vp(i,ireflector).gt.8.0)then
                  write(bo,*)'WARNING ::: velocity ABOVE reflector is',
     ...
     &                       ' greater than 8.0 km/s  .....'
            ...
            if(.not.single_turbo)then
             write(bo,*)'WARNING:'
       call adlineb(bo)
             write(bo,*)'Reflector indicated in velocity-model is',
     &                   ' marked with a : ',reflch
             write(bo,*)'Only   m   or   M   are allowed!'
       call adlineb(bo)
      ...
      if(j.eq.1)then
         if(.not.single_turbo)then
            write(bo,'(2x,i2,3x,2f7.2,1x,f7.2,1x,a1,1x,a40)') j,
     &                vp(i,j),hp(i,j),vdamp(i,j),reflch,titl
       call adlineb(bo)
         ...
         if(.not.single_turbo)then
            write(bo,13) j,vp(i,j),hp(i,j),vdamp(i,j),reflch
       call adlineb(bo)
...
c
      if(j.gt.1.and.hp(i,j).lt.0.0)then
         ...
         if(.not.single_turbo)then
            write(bo,*)'WARNING:'
       call adlineb(bo)
            write(bo,*)'Only top of the first layer can be negative !!'
         endif
      endif
c
    9 continue
...
14    continue
      i f (ireflector.ne.0) t h e n
         ...
         write(bo,*)
       call adlineb(bo)
        write(bo,'(1x,''Phases in the input-phaselist marked with a  '',
...
c
      if(.not.single_turbo)then
       ...
       call adlineb(bo)
         ...
         write(bo,'(//)')
       call adlineb(bo)
         write(bo,*)'Station-parameters read from file :'
...
c
      ...
      if(.not.single_turbo)then
         ...
         if(cns.eq.'S') xla(nsta)=-xla(nsta)
         if(cew.eq.'E') xlo(nsta)=-xlo(nsta)
         write(bo,27) nsta,stn(nsta),xla(nsta),cns,xlo(nsta),cew,
     &                ielev(nsta),dx,dy,dz,
...
c
      if(.not.single_turbo)then
         if(seismofilename.ne.' ')then
            write(bo,*)
       call adlineb(bo)
            write(bo,*)'Seismo-parameters read from file :'
            write(bo,*) seismofilename
       call adlineb(bo)
            write(bo,*)
            write(bo,*)'This file contains station-informations which'
       call adlineb(bo)
            write(bo,*)'are used for magnitude-determination'
            write(bo,*)'(seismometer-constant,filterparameters, etc.)'
       call adlineb(bo)
            write(bo,*)
            write(bo,*)
       call adlineb(bo)
            ...
            write(bo,*)
       call adlineb(bo)
            write(bo,*)'NO Seismo-parameter-file specified'
            write(bo,*)'--> NO magnitudes calculated!'
       call adlineb(bo)
            write(bo,*)
            write(bo,*)
       call adlineb(bo)
...
22    continue
      RETURN
c
 9910 call OPENERROR('inputparam','control-input-file FOR010',bo)
      return
 9911 call OPENERROR('inputparam','model-input-file (FOR010)',bo)
      return
 9912 call OPENERROR('inputparam','station-input-file (FOR010)',bo)
      return
 9913 call OPENERROR('inputparam','seismo-input-file (FOR010)',bo)
      return
c
      end ! of subr. inputparam
c
c
      subroutine INPUTDATA(i,bo)  ! old name:  INPUT2
      character*(*)  bo
c
...
2222           continue
 ...
 222        jjmin=jjmin1  ! first station in data which is on station-list!!
            ...
            enddo
cc          write(bo,'('' first station is '',a4)') sta(jjmin)
       call adlineb(bo)
            jjmin1=0
            do jj=1,nsta
     ...
     &                      sta(j),cphase(j)
                  ...
                  if(ireflector.eq.0)then
                     i f (.not.single_turbo) t h e n
                        write(bo,*)'WARNING:'
       call adlineb(bo)
                        write(bo,*)'subr. INPUTDATA >>> Phase is : ',
     &		             cphase(j)
                        write(bo,*)'but ireflector is: ',ireflector
       call adlineb(bo)
...
c
   ...
   16 continue
      if(.not.single_turbo)then
         if(isingle.eq.0) write(bo,*)'WARNING:     Event # ',i
       call adlineb(bo)
         if(isingle.gt.0) write(bo,*)'WARNING:     Event # ',isingle
         write(bo,'('' WARNING:  Station: >>>'',a4,
     &              ''<<< not found in stationlist!'')') sta(j)
       call adlineb(bo)
         write(bo,*)'Phase therefore skipped'
      endif
cc      write(6,*)'Event # ',i
...
17    continue
      ...
      if(ss1.gt.dmax)then
         if(.not.single_turbo)then
            write(bo,*)'WARNING:'
       call adlineb(bo)
            write(bo,'('' epicentral distance:'',f6.1,
         ...
         if(k.eq.istm(ll,i).and.ipwt(ll).lt.4)then
            ...
            if(sphase(ll,i).eq.itest)then  ! twice the same phase in this event
               ...
               if(.not.single_turbo)then
                 write(bo,*)'WARNING:'
       call adlineb(bo)
                 write(bo,*)'PHASETEST: POSSIBLE ERROR in phaselist !!'
                 write(bo,'(1x,''---> '',3i2.2,1x,2i2.2)')
     &                    iyr(i),imo(i),iday(i),ihr(i),imin(i)
       call adlineb(bo)
                 write(bo,*)'Event=',ie,' Obs-nr. = ',j,' >>> Station ',
...
cek print P and S number of obs
           if(nsp.eq.2) then
            write(bo,'(1x,i3,1x,3i2.2,1x,2i2.2,1x,f5.2,1x,f7.4,a1,
...
c
  ...
  999 call OPENERROR('inputdata','SHOT-data-input-file FOR009',bo)
      return
c
      end ! of subr. inputdata
c
      subroutine STATISTICSOUT(bo)
      character*(*) bo
c
...
chrm  floating point exceptions in case of rlen = 0
	 ...
	 refraylen(i) = 0.0
      ...
      write(bo,*)
       call adlineb(bo)
      write(bo,*)
      write(bo,*)'RAY-STATISTICS FOR  L A S T  ITERATION'
       call adlineb(bo)
      write(bo,*)'--------------------------------------'
      write(bo,*)
       call adlineb(bo)
      write(bo,*)'NHYP : nr of hypocenters in this layer'
      write(bo,*)'NREF : nr of headwaves in this layer'
       call adlineb(bo)
      write(bo,*)'%len : % of "refracted km" in this layer '
      write(bo,*)'       with respect to all refracted kilometers'
       call adlineb(bo)
      write(bo,*)'NHIT : nr of rays passed thru this layer'
      write(bo,*)'xy-km: average horizontal ray length [km] in layer'
       call adlineb(bo)
      write(bo,*)'z-km : average  vertical  ray length [km] in layer'
      write(bo,*)'RFLX : number of reflections at bottom of this layer'
       call adlineb(bo)
      write(bo,*)
      write(bo,'(a)')' nlay   top ..... bottom     velocity   '//
     &                'NHYP NREF %len  NHIT xy-km  z-km  RFLX'
      ...
      do i=1,nltot-1
         ...
         write(bo,'(2x,i2,3x,f6.2,''...'',f6.2,'' km   '',
...
c
      write(bo,*)
       call adlineb(bo)
      write(bo,'('' Total nr of events was '',i4)') isour
      write(bo,*)
       call adlineb(bo)
      write(bo,'('' Total nr of refracted rays = '',i5)') irefr
      ...
      write(bo,'(''                              ------'')')
       call adlineb(bo)
      write(bo,'('' Total nr of    all    rays = '',i5)') itotal
      write(bo,*)
       call adlineb(bo)
     ...
     &           err
      ...
      write(bo,'(1x,'' Refracted           rays : '',f7.2,'' meters'')')
     &           err
       call adlineb(bo)
      err=0.0
      if(irefl.gt.0) err=reflerr/irefl
      write(bo,'(1x,'' Reflected           rays : '',f7.2,'' meters'')')
     &           err
       call adlineb(bo)
      write(bo,*)
      write(bo,*)
      ...
      write(bo,*)'ALL RAYS TOGETHER:'
       call adlineb(bo)
      write(bo,'(1x,''Average horizontal ray length = '',
     &           f6.1,'' km   (Hypocenter --> Station) '')')
     &           avhraylen
      avvraylen=avvraylen/itotal
      write(bo,'(1x,''Average  vertical  ray length = '',
...
c
      write(bo,*)
       call adlineb(bo)
      write(bo,*)
      write(bo,*)'... and some more STATISTICS '
       call adlineb(bo)
      write(bo,*)'---------------------------- '
      ...
      write(bo,'(1x,''GAPs were between '',i3,'' and '',i3)')
     &           nint(mini),nint(maxi)
       call adlineb(bo)
      write(bo,'(''      (average GAP was '',i3,'')'')') iavgap
      write(bo,*)
       call adlineb(bo)
c
      write(bo,*)
       call adlineb(bo)
      write(bo,*)
      write(bo,*)' MAGNITUDES of INPUT-DATA:'
       call adlineb(bo)
      write(bo,*)
      write(bo,*)'Magnitude (# of events)   ***average number of obs***'
       call adlineb(bo)
      write(bo,*)
      lesseq1=0
      mge6=0
...
c
      write(bo,*)
       call adlineb(bo)
      write(bo,*)
      write(bo,*)' DEPTHs of INPUT-DATA and of LAST ITERATION:'
       call adlineb(bo)
      write(bo,*)' ==========================================='
      write(bo,*)
       call adlineb(bo)
      write(bo,*)'      Depth       # of events  '
      write(bo,*)
       call adlineb(bo)
      lesseq1=0
      mge6=0
      ...
      do i=1,105
         ...
         if(depthnri(i).gt.0)then
            write(bo,'(1x,''DEPTH ( input ) = '',f4.0,'' km :  '',a)')
     ...
     &                depthi,' '
       call adlineb(bo)
         ...
         if(depthnrl(i).gt.0)then
            write(bo,'(1x,''DEPTH (last_IT) = '',f4.0,'' km :  '',a)')
...
cc         write(bo,*)
      ...
      if(mge5i.gt.0)then
         write(bo,'(1x,''DEPTH ( input ) > 100. '',''km :  '',a)')
     &              cstari(1:mge5i)
       call adlineb(bo)
      else
         write(bo,'(1x,''DEPTH ( input ) > 100. '',''km :  '',a)')
     &              ' '
      ...
      if(mge5l.gt.0)then
         write(bo,'(1x,''DEPTH (last_IT) > 100. '',''km :  '',a)')
     &              cstarl(1:mge5l)
       call adlineb(bo)
      else
         write(bo,'(1x,''DEPTH (last_IT) > 100. '',''km :  '',a)')
     &              ' '
      ...
      write(bo,*)
c
      write(bo,*)'Residuals of the stations according to the azimuth:'
       call adlineb(bo)
      write(bo,*)'(RES  = total average residual at station)'
      write(bo,*)'(RES1 = average residual of rays from 1st quadrant)'
       call adlineb(bo)
      write(bo,*)'(RES2 = average residual of rays from 2nd quadrant)'
      write(bo,*)'(RES3 = average residual of rays from 3rd quadrant)'
       call adlineb(bo)
      write(bo,*)'(RES4 = average residual of rays from 4th quadrant)'
      write(bo,*)
       call adlineb(bo)
      write(bo,'(1x,''Stn#  Stn     RES          RES1        '',
     &           '' RES2         RES3         RES4'')')
      do i=1,nsta
         res0=0.0
         res1=0.0
     ...
     &                       +stnazires(i,8)*res4) / tot
         if(tot.gt.0.0)then
            ...
            write(bo,'(1x,i3,3x,a4,1x,5(f7.2,''('',i4,'')''))')
     &      i,stn(i),res0,ntot,res1,i1,
     &      res2,i2,res3,i3,res4,i4
       call adlineb(bo)
         else
            write(bo,'(1x,i3,3x,a4,1x,''-.-'')') i,stn(i)
       call adlineb(bo)
         endif
      enddo
      write(bo,*)
...
c
      ...
      if(icoordsystem.eq.2)then
         write(bo,37) i,e(1,i),-e(2,i),(e(j,i),j=3,4),rms(i),avres(i),
     &             b(j1), -b(j1+1), +b(j1+2), b(j1+3)      ! dot  dx  dy  dz
       call adlineb(bo)
      else
         write(bo,37) i,(e(j,i),j=1,4),rms(i),avres(i),
...
c
      ...
      write(bo,*)
       call adlineb(bo)
      write(bo,'('' A V E R A G E   of ADJUSTMENTS :'',19x,4f7.3)')
     &           avdt,avdx,avdy,avdz
      write(bo,'('' A V E R A G E   of ABSOLUTE ADJUSTMENTS :'',10x,
...
c
         do jjj=j11,j22
            ...
            write(bo,63) vp(i,kj),b(jjj),hp(i,kj),reflch
       call adlineb(bo)
...
c
      if(.not.single_turbo)then
         write(bo,'(1x,i3,1x,3i2.2,1x,2i2.2,f6.2,
...
ccc      if(.not.single_turbo) write(bo,*)
       call adlineb(bo)
      if(isingle.ne.0)then
         if(.not.single_turbo)then
          write(bo,'(1x,''Event# '',i3,'' GAP = '',i3)')
     &	  isingle,igap(i)
       call adlineb(bo)
         endif
         if(icoordsystem.eq.2.and.nreg.ge.1000)then
            if(.not.single_turbo)then
               write(bo,'(1x,a32,''   L+T Nr.: '',i4)') 
     &      regionname,nreg
       call adlineb(bo)
            endif
...
c
      subroutine OPENERROR(subr,char,bo)
      character*(*) bo
c
      implicit none
      include 'vel_com.f'
...
c
      subroutine CHECKSOLUTION(istopflag,better,bo)
      character*(*) bo
c
...
c
      ...
      if(.not.single_turbo)then
       ...
       call adlineb(bo)
         if(ibackups.eq.0)then
            write(bo,'('' Iteration nr '',i2,'' obtained:'')') nitt
       call adlineb(bo)
         else
            write(bo,'(''(Iteration nr '',i2,'')   BACKUP nr '',i1,
     &                 '' obtained:'')') nitt,ibackups
       call adlineb(bo)
         endif
         write(bo,10) davar1,xmsqrs1,sqrt(xmsqrs1)
       call adlineb(bo)
 10      format(' DATVAR=',f12.6,' mean sqrd residual= ',f12.6,
     &          '  RMS RESIDUAL= ',f12.6)
      endif
c
      if(isingle.ne.0) call GAPCALC(1,bo)
c
      if(isingle.eq.0) call AVRESISTATIST(bo)
c
...
c
      if(isingle.ne.0)then ! stop calc. for single event if changes get small
         if(nitt.gt.2.and.abs(datvar-davar1).lt.1e-6)then
            istopflag=1
            if(.not.single_turbo)then
               write(bo,*)'Changes in datvar < 1e-6   : STOPPING...'
       call adlineb(bo)
            endif
         endif
      endif
...
c
         if(.not.single_turbo)then
        write(bo,'('' *** WARNING: the data variance has increased.'')')
       call adlineb(bo)
         endif
        decreasing=1
        goto 1
...
c
      ...
      subroutine TRAVELTIME(i,nobs,iresflag,bo)
      character*(*) bo
c
...
c
  5   call TRAVDERIV('direct',
     &            nl,mll,v1,vsq1,rp,nrp,x2,y2,z2,ss,r1,r2,i,nobs,bo)
      goto 40
...
c
      subroutine ADJUSTMODEL(damp,bo)
      character*(*) bo
c
...
c     &                  'depth-adjustment := 0.0 for event ',i
         ...
         if(abs(b(4)).gt.zadj)then
            isconstrain(3)=1
c            write(bo,*)' *** depth-adjustment constrained for event ',i
       call adlineb(bo)
            if(b(4).gt.0.0)then
...
c
         e(4,i)=zmin
         iconstrain(i)=1
c         write(bo,'('' ***** depth constrained for event '',i5)') i
       call adlineb(bo)
      endif
c
...
c
       ...
       call adlineb(bo)
         ...
         do i=1,legs
            if(abs(avres(i)).gt.1.0)then
               write(bo,'('' Event# '',i3,'' >>> '',1x,3i2.2,1x,2i2.2,
     &                    ''  AVRES ='',f6.2,'' NOBS ='',i3)')
     ...
     &                    avres(i),knobs(i)
       ...
       call adlineb(bo)
            ...
            write(bo,*)'ZERO events of this kind found! (lucky guy!!!)'
       call adlineb(bo)
         endif
         write(bo,*)
       call adlineb(bo)
...
c
     ...
     &           '']'')')nrreflres,abreflres,avreflres
      ...
      write(bo,'(1x,i5,'' ALL                 RAYS ='',f9.5,'' ['',f9.5,
...
5     format(1x,3f7.3,3x,a1)
      ...
      if(.not.single_turbo)then
         write(bo,5) vp(k2,k),b(jjj),hp(k2,k),reflch
       call adlineb(bo)
      endif
   4  continue
c
...
7     continue
      if(.not.single_turbo)then
         write(bo,8) (stn(m),ptcor(m),cc(m),m=1,nsta)
       call adlineb(bo)
...
28    continue
      if(.not.single_turbo)then
         write(bo,8) (stn(m),stcor(m),cc(m),m=1,nsta)
       call adlineb(bo)
      endif
511   continue
      if(.not.single_turbo)then
...
c
      subroutine RESOLCOVAR(davari,bo)
      character*(*) bo
c
...
ccc   19 rht(j)=rht(j)*davari*scale1
 ...
 20         format(1x,'covariance row',i3)
       ...
       call adlineb(bo)
         ...
         if(.not.single_turbo) write(bo,*) pcard
       call adlineb(bo)
         do j=1,4
            COVc(l,j)=rht(j)
         enddo
...
ccc     &            ''D-spread ='',f6.3)') (Rs(j,j),j=1,nef) , spread3
      ...
      if(.not.single_turbo) write(bo,22) (s(j),j=1,nef)
       call adlineb(bo)
 22   format(8f10.4)
      if(.not.single_turbo)then
         write(bo,*)
...
c
     ...
     &           rp,nrp,x2,y2,z2,ss,r1,r2,ievent,inobs,bo)
      character*(*) bo
c
...
c
      subroutine REJECTOBS(i,nobs,iresflag,bo)
      character*(*) bo
c
...
c
      ...
      subroutine REVIVEOBS(i,nobs,iresflag,bo)
      character*(*) bo
c
...
c
      ...
      do i=1,nsta
         if(nactualsta(i).gt.0)then
            if(.not.single_turbo)then
               if(nsp.eq.2) then
               write(bo,'('' readings for station '',a4,'' : tot='',
     &                  i4,''  P:'',i4,''  S:'',i4)')
     &                  stn(i),nactualsta(i),nobsp(i),nobss(i)
       call adlineb(bo)
               else
               write(bo,'('' readings for station '',a4,'' :'',i4)')
     &                  stn(i),nactualsta(i)
       call adlineb(bo)
               endif
            endif
...
c
      do 2 i=1,legs
       ...
       call adlineb(bo)
         ...
         if(isingle.eq.0)then
          write(bo,'(1x,''Station residuals for event='',i4,
...
52       continue
         write(bo,53) (smn(j,i),phzz(j),kpwt(j,i),res(j,i),tctime(j,i),
     &                 del(j),j=1,knobs(i))
       call adlineb(bo)
...
c
         ...
         if(bb(m).lt.-0.1)then
           iwarn=1
           write(bo,'(5x,''WARNING: Station = '',a4,
...
cek         if(iwarn.eq.0)then
         write(bo,'(1x,a4,3x,a1,i4,3f8.4,1x,2f8.4)') 
     &             stn(m),phz(2),icc(m),ee(m),aa(m),bb(m),dd(m),ptcor(m)
       call adlineb(bo)
cek         else
cek            write(bo,'(1x,a4,3x,a1,i4,2f8.4,''   ?.?.?''1x,2f8.4,
       call adlineb(bo)
cek     &              '' <--- !!'')')
...
cek         write(bo,'(1x,a4,3x,a1,i4,25x,f8.4)') 
       call adlineb(bo)
...
c
      ...
      subroutine SINGULARVALUES(i,bo)
      character*(*) bo
c
...
c
      call direct1(nl,v,vsq,thk,jl,tkj,delta,depth,tdir,salpha,deljl,bo)
...
c
      subroutine DIRECT1(nl,v,vsq,thk,jl,tkj,delta,depth,tdir,u,x,bo)
      character*(*) bo
c
